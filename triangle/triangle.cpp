...
public:
	...
	{
        // assert断言，如果里面的条件错误，则终止运行，输出源文件错误代码和行号
    ...
    // 为每一次帧缓冲图像创建分离的命令池，多线程是vulkan最大的优势之一，而OpenGL一次记录所有渲染命令到命令池，重新提交到队列。是单线程操作。
    ...
    // render pass的设置，render pass是vulkan的新概念，有很多附件，每个附件包含多个subpasses。驱动知道预渲染结果，从而适合优化tile-based rendering
	// Render pass setup
